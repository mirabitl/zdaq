...
namespace zdaq {
  ...
  public:
    monitorApplication(std::string name) : zdaq::baseApplication(name) ,_running(false),_context(NULL),_publisher(NULL),_period(120)
    {
      ...
      // Commands
      _fsm->addCommand("IDENTITY",boost::bind(&zdaq::monitorApplication::c_identity,this,_1,_2));
      _fsm->addCommand("INFOS",boost::bind(&zdaq::monitorApplication::c_infos,this,_1,_2));
    
    }
    void c_infos(Mongoose::Request &request, Mongoose::JsonResponse &response)
    {
 
      response["INFOS"]=this->status();
    }
    void c_identity(Mongoose::Request &request, Mongoose::JsonResponse &response)
    {
      std::stringstream sheader;
      sheader<<this->hardware()<<"@"<<this->parameters()["location"].asString();

      response["INFOS"]=sheader.str();
    }
      ...
      while (_running)
	{
	  ...
	  // ID
	  std::stringstream sheader;
	  sheader<<this->hardware()<<"@"<<this->parameters()["location"].asString()<<"@"<<time(0);
